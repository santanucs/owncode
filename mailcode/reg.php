<?php
session_start();
include_once('conn-test.php');include_once('class.apimailer.php');
include_once('functions.php');
include_once('phpmailer.php');//echo $_SERVER["HTTP_HOST"];
// INCLUDE THE phpToPDF.php FILE
require("finalone/phpToPDF.php"); 



/*

$to = "debajyoti.p@gsl.in";		
	$header .= "MIME-Version: 1.0\r\n";
	$header .= "Content-Type: text/html; charset=\"utf-8\"\r\n";
	$header .= "Content-Transfer-Encoding: 7bit\r\n";
	$header .= "From: TEST NAME <" . "Emails Demo"  . ">\r\n"; 		
	$subject = "Free Quote Mail";		
	$content = '';
	$content.= '';		
	$content.= '';	
	$content.= '';	
...
'.$messages.' Message : 
'.$email.' ';	
$ResultMail = mail($to,$subject,$content,$header);
var_dump($ResultMail);exit();




					#	Start - MAIL SEND TO THE VENDOR WITHT THE PDF ATTACHMENT
$ApplicationID = 'PXFGB-58379-RR7Kt';
					...
					$ArrayParam['PDF_NAME_FULLPATH'] = 'upload/'.$ApplicationID.'.pdf';
					$ArrayParam['MAIL_SUBJ'] = $ApplicationID.' - Vmart Registration';
					
					
					$ArrayParam['MAIL_BODY'] =  'TEST TEST TEST';
...
{
	if($PaymentMode != '' && $PaymentStatus != '' && $PaymentAmount >= 16854 && $VendorEmail != '' && $VendorName != '')
	{
		...
		if($NumRowsDuplicate < 1)
		{
			...
			if($LastInsertID > 0)
			{
				...
				$my_html = file_get_contents("finalone/vmart_vendor/pdf.html");
				$my_html = str_replace('VARIABLE_VENDOR_NAME',$VendorName,$my_html );
				$my_html = str_replace('VARIABLE_APPLICATION_ID',$ApplicationID,$my_html );
				$my_html = str_replace('PAYMENT_STATUS',$PaymentStatus,$my_html );
				$my_html = str_replace('PAYMENT_MODE',$PaymentMode,$my_html );
				...
				$pdf_options = array(
				  ...
				  "file_name" => $ApplicationID.'.pdf');
				
				// CALL THE phpToPDF FUNCTION WITH THE OPTIONS SET ABOVE
				phptopdf($pdf_options);
				#	End - COMPLETED CREATE PDF				
...
Malad (E), Mumbai - 400 097
					...
					';
					$ArrayParam['MAIL_BODY'] =  $my_html;										if($_SERVER["HTTP_HOST"] == "staging.ginesys.in"){						$ResultMailSend = MailSendViaSendgrid($ArrayParam);					}					else{
						$ResultMailSend = MailSendViaAuthentication($ArrayParam);					}
					if($ResultMailSend == 'Message sent') $Msg .= ' & Mail sent Successfully';
					else $Msg .= ' But Failed to send mail ( '.$ResultMailSend .' ) ' ;
					#	End - MAIL SEND TO THE VENDOR WITHT THE PDF ATTACHMENT
				if(file_exists('upload/'.$ApplicationID.'.pdf'))
				{
				}
				else $Msg .= ' . Failed to send the attachemnt.' ;
			}
		}
		else
...
if($SubmitVendorFormAction == 'Yes')
{
	...
	if($VendorPerson != '')
	{
		if($SqlWHERE != '')
			$SqlWHERE .= ' AND ';
		$SqlWHERE .= " TablVenvorMast.BEMAIL = '".$VendorPerson."'  ";
	}
	if($VendorAddress != '')
	{
		if($SqlWHERE != '')
			$SqlWHERE .= ' AND ';
		$SqlWHERE .= " TablVenvorMast.BADDR LIKE '%".$VendorAddress."%'  ";
	}
	if($VendorEmail != '')
	{
		if($SqlWHERE != '')
...
#		$Sql = " SELECT * FROM vmartreg_source_master AS TablVenvorMast $SqlWHERE AND SLCODE NOT IN (SELECT VmartReg.SLCODE FROM vmartreg_registration AS VmartReg ) ";
...
{
	if(trim(document.getElementById('VendorName').value) == '')
	{
...
<?php if($Msg != ''){ ?>alert('<?php echo $Msg;?>');window.location.href = 'reg.php';<?php echo '</script>';exit(); } ?>
...
<tr>
<td>
<a href="reg.php"><img src="http://staging.ginesys.in/vmartreg/finalone/vmart_vendor/header.png"  /></a>
</td>
</tr>
<tr>
<td>
...
<td style="width:40%; padding:3px 0px;"><?php echo $Fetch['SLNAME'];?></td>
</tr>
<tr>
<td valign="top" style="width:25%;padding:3px 0px;">Contact Email:</td>
<td style="width:40%;padding:3px 0px;"><?php echo $Fetch['BEMAIL'];?></td>
</tr>
<tr>
<td valign="top" style="width:25%;padding:3px 0px;">Contact Number:</td>
...
<tr>
<td valign="top" style="width:25%;padding:10px 0px;">Contact Email:</td>
<td style="width:40%;padding:10px 0px;"><input type="text" name="VendorPerson" id="VendorPerson" value="<?php echo $VendorPerson;?>" /></td>
</tr>
<tr>
<td valign="top" style="width:25%;padding:10px 0px;">Contact Number:</td>
...
<td style="width:40%;padding:10px 0px;"><input type="text" name="VendorEmail" id="VendorEmail" value="<?php echo ($VendorEmail == '')?$VendorPerson:$VendorEmail;?>" /></td>
</tr>
<tr>
...
<td style="width:25%;padding:10px 0px;">
<input name="PaymentMode" id="PaymentMode" type="radio" value="Chq" <?php if($PaymentMode == 'Chq' || $PaymentMode == ''){ ?>checked="checked"<?php } ?> /> Cheque 
...
<td style="width:25%;padding:10px 0px;">
16,854 ( Service charge applied )
</td>
</tr>
<tr>
  <td style="width:25%;padding:10px 0px;"><input type="button" name="ButtonCancel" id="ButtonCancel" value="Clear / Cancel" onclick="CheckClear();" /></td>
...
<input type="hidden" name="PaymentAmount" id="PaymentAmount" value="16854" />
</form>
</tr>
<?php } ?>
