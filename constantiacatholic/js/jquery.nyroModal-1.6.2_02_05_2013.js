 ...
 * jQueryDate: 2010-02-23 (Tue, 23 Feb 2010) jQuery
...
jQuery(function(jQuery) {
	...
	jQuery.fn.nyroModal = function(settings) {
	...
	jQuery.fn.nyroModalManual = function(settings) {
	...
	jQuery.nyroModalManual = function(settings) {
	...
	jQuery.nyroModalSettings = function(settings, deep1, deep2) {
	...
	jQuery.nyroModalRemove = function() {
	...
	function processModal(settings) {
		...
		callingSettings = jQuery.extend(true, settings);
	...
	function setDefaultCurrentSettings(settings) {
		...
		currentSettings = jQuery.extend(true, {}, jQuery.fn.nyroModal.settings, settings);
	...
	function setCurrentSettings(settings, deep1, deep2) {
		...
		if (modal.started) {
			...
			if (deep1 && deep2) {
				...
				jQuery.extend(true, jQuery.fn.nyroModal.settings[deep1][deep2], settings);

			} else if (deep1) {

				jQuery.extend(true, jQuery.fn.nyroModal.settings[deep1], settings);

			} else {

				jQuery.extend(true, jQuery.fn.nyroModal.settings, settings);
	...
	function setTitle() {
		...
		var title = jQuery('h1#nyroModalTitle', modal.contentWrapper);
	...
	function initModal() {
		...
		if (!modal.full) {
			...
			if (currentSettings.blocker) {
				...
				var h = jQuery(window).height()+'px';

				if (jQuery(window).height() >= body.outerHeight()) {
				...
				iframeHideIE = jQuery('<iframe id="nyroModalIframeHideIe" src="javascript:\'\';"></iframe>')
			...
			contain.append(jQuery('<div id="nyroModalFull"><div id="nyroModalBg"></div><div id="nyroModalWrapper"><div id="nyroModalContent"></div></div><div id="nyrModalTmp"></div><div id="nyroModalLoading"></div></div>').hide());



			modal.full = jQuery('#nyroModalFull')

				.css(full)

				.show();

			modal.bg = jQuery('#nyroModalBg')
			...
			modal.loading = jQuery('#nyroModalLoading')

				.css(currentSettings.cssOpt.loading)

				.hide();

			modal.contentWrapper = jQuery('#nyroModalWrapper')

				.css(currentSettings.cssOpt.wrapper)

				.hide();

			modal.content = jQuery('#nyroModalContent');
			...
			if (!currentSettings.blocker && currentSettings.windowResize) {
				...
				jQuery(window).bind('resize.nyroModal', function() {
	...
	function windowResizeHandler() {
		...
		jQuery.nyroModalSettings(initSettingsSize);
	...
	function fillContent() {
		...
		jQuery(currentSettings.openSelector, modal.contentWrapper).nyroModal(getCurrentSettingsNew());
	...
	function wrapContent() {
		...
		if (currentSettings.gallery) {
			...
			if (linkPrev) {
				...
				var prev = jQuery('.nyroModalPrev', modal.contentWrapper)
				...
				if (isIE6 && currentSettings.type == 'swf') {
					...
					prev.before(jQuery('<iframe id="nyroModalIframeHideIeGalleryPrev" src="javascript:\'\';"></iframe>').css({
				...
				jQuery('.nyroModalPrev', modal.contentWrapper).remove();
			...
			if (linkNext) {
				...
				var next = jQuery('.nyroModalNext', modal.contentWrapper)
				...
				if (isIE6 && currentSettings.type == 'swf') {
					...
					next.before(jQuery('<iframe id="nyroModalIframeHideIeGalleryNext" src="javascript:\'\';"></iframe>')
				...
				jQuery('.nyroModalNext', modal.contentWrapper).remove();
	...
	function calculateSize(resizing) {
		...
		modal.content.css(jQuery.extend({}, tmp.content, currentSettings.cssOpt.content));

		modal.wrapper.css(jQuery.extend({}, tmp.wrapper2, currentSettings.cssOpt.wrapper2));



		if (!resizing)

			modal.contentWrapper.css(jQuery.extend({}, tmp.wrapper, currentSettings.cssOpt.wrapper));
	...
	function removeModal(e) {
		...
		if (modal.full && modal.ready) {
			...
			if (!currentSettings.blocker)
				...
				jQuery(window).unbind('resize.nyroModal');
			...
			if (modal.loadingShown || modal.transition) {
				...
				jQuery('iframe', modal.content).hide(); // Fix issue 359
	...
	function ajaxLoaded(data) {
		...
		if (currentSettings.selector) {
			...
			data = jQuery('<div>'+data+'</div>').find(currentSettings.selector).html()
	...
	function endShowContent() {
		...
		if (shouldResize) {
			...
			jQuery.nyroModalSettings({width: currentSettings.setWidth, height: currentSettings.setHeight});
	...
	function filterScripts(data) {
		...
		jQuery.each(jQuery.clean({0:data}, this.ownerDocument), function() {

			if (jQuery.nodeName(this, "script")) {

				if (!this.src || jQuery(this).attr('rel') == 'forceLoad') {

					if (jQuery(this).attr('rev') == 'shown')
	...
	function getCurCSS(elm, name) {
		...
		var ret = parseInt(jQuery.curCSS(elm, name, true));
	...
	function debug(msg) {
		...
		if (jQuery.fn.nyroModal.settings.debug || currentSettings && currentSettings.debug)
	...
	jQuery(jQuery.fn.nyroModal.settings.openSelector).nyroModal();
